package com.gopay.money.query.adapter.out.aws.dynamodb;

import com.gopay.money.query.adapter.axon.QueryMoneySumByAddress;
import com.gopay.money.query.application.port.out.GetMoneySumByAddressPort;
...
public class DynamoDBAdapter implements GetMoneySumByAddressPort, InsertMoneyIncreaseEventByAddress {
    ...
    public void insertMoneyIncreaseEventByAddress(String addressName, int moneyIncrease) {
        ...
        // PK: 강남구#오늘날짜 SK: 5,000, balance, 5,000
//        String pk = addressName + "#" + LocalDate.now().format(DateTimeFormatter.ofPattern("yyMMdd"));
        String pk = addressName + "#" + "20250623";
        String sk = String.valueOf(moneyIncrease);
        putItem(pk, sk, moneyIncrease);

        // 2. 지역 정보 잔액 증가시켜야 해요. (Query, Update)
        // 2-1. 지역별/일별 정보
        //  - PK: 강남구#오늘날짜#summary SK: -1 balance: + 5,000
    ...
    private void putItem(String pk, String sk, int balance) {
        try {
            ...
            attrMap.put("PK", AttributeValue.builder().s(pk).build()); // s - string
            attrMap.put("SK", AttributeValue.builder().s(sk).build());
            attrMap.put("balance", AttributeValue.builder().n(balanceStr).build()); // n - number
